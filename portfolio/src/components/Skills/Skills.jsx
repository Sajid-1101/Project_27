...
const Skills = () => {
  return (
    <div id='Skills' className='scroll-smooth scroll-mt-16 text-center text-white mb-16 px-4 sm:px-8 lg:px-20'>
      ...
      <div className=' flex justify-center  flex-wrap gap-8 '>
        ...
        <div key={i} className='border-2 border-purple-500 shadow-lg shadow-purple-500/50 rounded-2xl min-h-70 max-h-120 min-w-60 max-w-160 hover:bg-purple-50/10  transition-colors duration-200' >
          ...
          <div className='flex flex-wrap justify-center'>
            {si.skills.map((s,idx)=>(
              <div key={idx} className='skill-float m-4 border rounded-2xl p-2 border-gray-300/10 transition-all duration-300 hover:border-purple-400 hover:shadow-lg hover:shadow-purple-500/30' style={{ animationDelay: `${idx * 0.18}s` }}>
              <span className='font-semibold text-white text-shadow-lg text-shadow-amber-50/10 text-sm'>{s.name}</span>
              <img src={s.logo} alt={`${s.name} logo`} className='h-10 mx-auto'/>
              </div>
            ))}
          </div>
        </div>
